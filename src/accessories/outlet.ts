...
export class OutletAccessory {
  ...
  private async handleOnGet(): Promise<CharacteristicValue> {
    ...
    try {
      ...
      this.log.warn(`WSK onGet failed for devicetypecd=${ctx.devicetypecd}: ${(err as Error).message}`);
      throw this.notResponding();
    }
    if (power === undefined) {
      // Empirically every WSK getdevice response carries operation[0].power.
      // A missing field signals an abnormal response; surface it as
      // "Not Responding" rather than silently coercing to off.
      this.log.warn(`WSK onGet: power field missing for devicetypecd=${ctx.devicetypecd}`);
      throw this.notResponding();
    }
    return power === 'on';
  }
