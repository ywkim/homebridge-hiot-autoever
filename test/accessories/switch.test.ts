...
const CONTEXT = {
  devicecd: 'SWT_TEST_001',
  devicetypecd: 'SWT',
  devicenm: 'all-off-test',
  spacenm: 'space-test',
};
...
function setup() {
  ...
  const handler = new SwitchAccessory(api as any, log as any, accessory as any, client as any);
...
describe('SwitchAccessory', () => {
  it('adds Switch service and registers only onSet for On (no onGet, background-poll pattern)', () => {
    const { svc, on } = setup();
  ...
  it('exposes devicecd for the poller', () => {
    const { handler } = setup();
    expect(handler.devicecd).toBe('SWT_TEST_001');
  });
  ...
  it('sets AccessoryInformation Manufacturer / Model / SerialNumber', () => {
    ...
    expect(calls).toContainEqual([CharId.Model, 'SWT']);
    expect(calls).toContainEqual([CharId.SerialNumber, 'SWT_TEST_001']);
  });

  it('reuses existing Switch service if already attached', () => {
    const api = makeApi();
    const log = makeLog();
    const accessory = makeAccessory({ ...CONTEXT });
    ...
    const client = makeClient();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    new SwitchAccessory(api as any, log as any, accessory as any, client as any);
  ...
  it('onSet(true) issues exeDeviceBatch with value "on"', async () => {
    ...
    expect(client.exeDeviceBatch).toHaveBeenCalledWith([
      { devicecd: 'SWT_TEST_001', resource: 'operation', attribute: 'power', value: 'on' },
    ]);
  });
  ...
  it('onSet(false) issues exeDeviceBatch with value "off"', async () => {
    ...
    expect(client.exeDeviceBatch).toHaveBeenCalledWith([
      { devicecd: 'SWT_TEST_001', resource: 'operation', attribute: 'power', value: 'off' },
    ]);
  });
  ...
  it('does not log devicecd or devicenm at info level', () => {
    ...
    expect(visible).not.toContain('SWT_TEST_001');
    expect(visible).not.toContain('all-off-test');
  });
});
